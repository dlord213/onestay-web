...
export const resortAPI = {
  ...
  createResortWithImage: async (
    resortData: ResortFormData,
    token: string
  ): Promise<Resort> => {
    try {
      ...
      console.log(
        "- location (stringified):",
        JSON.stringify(resortData.location)
      );
      console.log("- description:", resortData.description);
      console.log(
        "- imageUri:",
        resortData.imageUri ? "Present" : "Not present"
      );
  ...
  updateResort: async (
    ...
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "Failed to update resort"
      );
    }
  },

  updateResortImage: async (
    resortId: string,
    imageUri: string,
    token: string
  ): Promise<Resort> => {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "Failed to update resort image"
      );
    }
  },
  ...
  getAllResorts: async (): Promise<Resort[]> => {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "Failed to fetch resorts"
      );
    }
  },
  ...
  searchResorts: async (query: string): Promise<Resort[]> => {
    try {
      const response = await apiRequest(
        `/resort/search?q=${encodeURIComponent(query)}`,
        {
          method: "GET",
        }
      );
      ...
      throw new Error(
        error instanceof Error ? error.message : "Failed to search resorts"
      );
    }
  },
  ...
  getResortsByOwner: async (
    ownerId: string,
    token: string
  ): Promise<Resort[]> => {
    try {
      const response = await authenticatedApiRequest(
        `/resort/owner/${ownerId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      throw new Error(
        error instanceof Error ? error.message : "Failed to fetch owner resorts"
      );
    }
  },
};
